...
<script>
    ...
    var offers = $('#offers').DataTable({
        ...
        ajax: {
            type: 'get',
            url: '{{ route('ajax.opportunity.offersDatatable') }}',
            data: function (d) {
                return $.extend({}, d, {
    ...
    getUsers({{ $opportunity->company_id }});
    getStocks({{ $opportunity->company_id }});
    getUnits({{ $opportunity->company_id }});
    getPayTypes({{ $opportunity->company_id }});
    getDeliveryTypes({{ $opportunity->company_id }});
    getOfferStatuses({{ $opportunity->company_id }});
