...
<script>
    ...
    var matches = $('#matches').DataTable({
        ...
        ajax: {
            ...
            data: function (d) {
                return $.extend({}, d, {
                    relation_type: 'App\\Models\\Country'
                });
            }
        },
    ...
    function getGuidCountries() {
        $.ajax({
            type: 'get',
            url: '{{ route('ajax.country.erp.index') }}',
            data: {},
            success: function (response) {
                guidCreate.empty();
                guidCreate.append(`<optgroup label=""><option value="">Seçim Yapılmadı</option></optgroup>`);
                $.each(response, function (i, data) {
                    guidCreate.append(`<option value="${data.ulke_Guid}">${data.ulke_ulkeadi}</option>`);
                });
    ...
    CreateButton.click(function () {
        ...
        if (relation_id == null || relation_id === '') {
            toastr.warning('CRM Ülke Seçmediniz');
        } else if (guid == null || guid === '') {
            toastr.warning('Ticari Program Ülke Seçmediniz!');
        } else {
            saveMatch({
                relation_type: 'App\\Models\\Country',
    ...
    DeleteButton.click(function () {
        var relation_type = 'App\\Models\\Country';
