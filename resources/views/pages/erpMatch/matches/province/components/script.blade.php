...
<script>
    ...
    var matches = $('#matches').DataTable({
        ...
        ajax: {
            ...
            data: function (d) {
                return $.extend({}, d, {
                    relation_type: 'App\\Models\\Province'
                });
            }
        },
    ...
    function getGuidProvinces() {
        $.ajax({
            type: 'get',
            url: '{{ route('ajax.province.erp.index') }}',
            data: {},
            success: function (response) {
                guidCreate.empty();
                guidCreate.append(`<optgroup label=""><option value="">Seçim Yapılmadı</option></optgroup>`);
                $.each(response, function (i, data) {
                    guidCreate.append(`<option value="${data.iller_Guid}">${data.iller_iladi}</option>`);
                });
    ...
    CreateButton.click(function () {
        ...
        if (relation_id == null || relation_id === '') {
            toastr.warning('CRM Şehir Seçmediniz');
        } else if (guid == null || guid === '') {
            toastr.warning('Ticari Program Şehir Seçmediniz!');
        } else {
            saveMatch({
                relation_type: 'App\\Models\\Province',
    ...
    DeleteButton.click(function () {
        var relation_type = 'App\\Models\\Province';
